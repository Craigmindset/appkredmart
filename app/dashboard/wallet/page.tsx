...
import { useUserWallet } from "@/lib/services/wallet/use-wallet";
...
export default function WalletPage() {
  ...
  const dataPlansForNetwork = useMemo(
    () => DATA_PLANS.filter((p) => p.code === dataNetwork),
    [dataNetwork]
  );
  ...
  const handleAirtimePurchase = async () => {
    const amount =
      typeof airAmount === "number" ? airAmount : Number(airAmount);
    if (!/^\d{11}$/.test(airPhone)) {
      toast({
        title: "Invalid phone",
        description: "Enter an 11-digit phone number",
        variant: "destructive",
      });
      return;
    }
    ...
    if (ok) {
      toast({
        ...
        description: `Sent ${formatMoneyColon(
          amount,
          currency
        )} to ${airPhone} (${airNetwork})`,
      });
      setAirAmount("");
      setAirPhone("");
    }
  ...
  const handleDataPurchase = async () => {
    ...
    if (ok) {
      toast({
        ...
        description: `Sent ${plan.label} for ${formatMoneyColon(
          plan.price,
          currency
        )} to ${dataPhone} (${dataNetwork})`,
      });
      setPlanId("");
      setDataPhone("");
    }
          ...
          >
              ...
              {mode === "airtime" ? (
                <div className="grid gap-4 md:grid-cols-3">
                  ...
                  <div>
                    ...
                    <Select
                      value={airNetwork}
                      onValueChange={(v: NetworkCode) => setAirNetwork(v)}
                    >
                      <SelectTrigger>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  ...
                  <div>
                    ...
                    <Select
                      value={dataNetwork}
                      onValueChange={(v: NetworkCode) => setDataNetwork(v)}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Select network" />
                      </SelectTrigger>
                          ...
                          <SelectItem key={p.label} value={p.label}>
                            <span className="inline-flex justify-between gap-3 w-full">
                              ...
                              <span className="text-muted-foreground">
                                {formatMoneyColon(p.price, currency)}
                              </span>
                            </span>
                          </SelectItem>
                        ))}
            ...
            <TabsContent value="bills" className="mt-4">
              <div className="rounded-lg border p-4 text-sm text-muted-foreground">
                Pay Bills is coming soon. You’ll be able to pay electricity, TV,
                internet and more directly from your wallet.
              </div>
            </TabsContent>
          </Tabs>
        </CardContent>
