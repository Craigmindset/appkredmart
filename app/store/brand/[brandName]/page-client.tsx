...
export default function BrandPage({ brandName }: { brandName: string }) {
  ...
  const handleFilterChange = useCallback(
    (next: {
      ...
      // If brand changes, navigate to new brand page or back to store
      if (next.brand !== actualBrand) {
        if (next.brand) {
      ...
      // Otherwise just update the filter state without navigation
      setFilter(next);
    },
    [actualBrand, router]
  );
  ...
  const allProducts = useMemo(() => {
    ...
    // Apply client-side sorting as backup/enhancement
      ...
      <section className="w-full px-1 sm:px-4 pt-6">
        <div className="w-full max-w-7xl mx-auto px-2 md:px-6">
          ...
          <div id="results" className="-mt-10">
            <ProductsGrid
              title={`${actualBrand} Products`}
              description={`Discover our collection of authentic ${actualBrand} products`}
