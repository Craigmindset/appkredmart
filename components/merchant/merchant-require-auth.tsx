...
export default function MerchantRequireAuth({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    console.log("MerchantRequireAuth - isHydrated:", isHydrated, "user:", user)

    if (isHydrated && !user) {
      const next = pathname || "/dashboard/merchant"
      console.log("Redirecting to admindesk with next:", next)
    ...
    return (
      <div className="flex h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Loading merchant session...</p>
        </div>
      </div>
    )
  }
